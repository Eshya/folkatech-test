...
const createUser = (data, res) => {
    return new Promise((resolve, reject) => {
        ...
        const findIdentity = Model.findOne({
...
exports.register = async (req, res, next) => {
    let {userName, emailAddress, accountNumber, identifyNumber} = _beforeSave(
        req.body
    );
    let result = [];
    try {
        const resultUser = await createUser(
            {userName, emailAddress, accountNumber, identifyNumber},
            res
        );
...
exports.updateById = async (req, res, next) => {
    ...
    try {
        ...
        if (data.emailAddress) {
            const findEmail = await Model.findOne({
        ...
        if (data.accountNumber) {
            const findNumber = await Model.findOne({
...
exports.login = (username, password) => {
    return new Promise((resolve, reject) => {
        if (
            username == process.env.DISPOSABLE_LOGIN_UNAME &&
            password == process.env.DISPOSABLE_LOGIN_PWD
        )
